...
class MultimediaBoxBlockController extends BlockController {
	...
	public function on_page_view() {
		$this->addHeaderItem("<link href='http://fonts.googleapis.com/css?family=Molengo' rel='stylesheet' type='text/css'>");
	...
	function get_thumb_src($file) {
		...
		if ($file->getAttribute('custom_thumbnail')) {
			$th = $i->getThumbnail($file->getAttribute('custom_thumbnail'),$this->thumb_size,$this->thumb_size);
			$url['src'] = $th->src;
			$url['width'] = $th->width;
		...
		elseif ($fv->hasThumbnail(1)) {
			$th = $i->getThumbnail($file ,$this->thumb_size, $this->thumb_size);
			$url['src'] = $th->src;
			$url['width'] = $th->width;
			$url['height'] = $th->height;
	...
	function save ($data) {
		...
		$ave['bID'] = $data['bID'];
		$ave['options'] = is_array($data['options']) ? implode(',',$data['options']) : $ave['options'];
		parent::save($ave);
	}
